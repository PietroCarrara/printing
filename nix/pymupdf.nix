# Imported from 343996e + fix from https://github.com/NixOS/nixpkgs/pull/334596 + skip tests

{ lib, stdenv, buildPythonPackage, pythonOlder, fetchFromGitHub, fetchpatch
, python,

# build-system
libclang, psutil, setuptools, swig,

# native dependencies
freetype, harfbuzz, openjpeg, jbig2dec, libjpeg_turbo, gumbo,

# dependencies
toPythonModule, mupdf,

# tests
pytestCheckHook, fonttools, pillow, pymupdf-fonts, }:
...
in buildPythonPackage rec {
  ...
  patches = [
    ...
    (fetchpatch {
      ...
      url =
        "https://github.com/pymupdf/PyMuPDF/commit/f42ef85058fee087d3f5e565f34a7657aad11240.patch";
  ...
  nativeBuildInputs = [ libclang swig psutil setuptools ];

  buildInputs = [ freetype harfbuzz openjpeg jbig2dec libjpeg_turbo gumbo ];
  ...
  checkInputs = [ fonttools pillow pymupdf-fonts ];
  ...
  pythonImportsCheck = [ "pymupdf" "fitz" ];
